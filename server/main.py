"""
NCEC AI Platform — PaddleOCR Backend Server
=============================================
...
# Lazy-load so the server starts fast; model downloads on first request.
_ocr_engine = None
...
# ── FastAPI app ─────────────────────────────────────────────────────────
app = FastAPI(title="NCEC OCR API", version="1.0.0")
...
def ocr_image(img_array: np.ndarray) -> str:
    ...
    lines = []
    for res in results:
        # PaddleOCR v3.7 returns a dict-like object with 'rec_texts'
...
async def upload_and_start_ocr(file: UploadFile = File(...)):
    ...
    Returns a job_id that can be polled for real-time results.
    """
    ...
    # Detect page count fast (before starting OCR)
    total_pages = 1
    ...
    jobs[job_id] = {
        ...
        "pct": 0,
        "status": "processing",
        "pages": {},       # page_number -> extracted text
        "error": None,
    }

    # Start OCR in a background thread (non-blocking)
...
async def get_job_status(job_id: str, page: Optional[int] = None):
    """
    Poll job progress. Optionally request a specific page's text.
    """
    job = jobs.get(job_id)
    if not job:
...
if __name__ == "__main__":
    import uvicorn
    print("🚀 NCEC OCR Server starting on http://localhost:8100")
    print("   Engine: PaddleOCR (Arabic + Multilingual)")
    print("   Endpoints:")
    print("     POST /api/ocr/upload     — Upload & start OCR")
    print("     GET  /api/ocr/status/:id — Poll results")
    print()
